...
/* 
		find:{
			async handler(ctx: { params: { id: any; }; }){
				// var qry = {name:"test1" }
				var dataById = new DbConnection1(this.adapter).getDataById(ctx.params.id)
				return dataById
			}
		},
 */
		...
		updateMany: {
			...
			handler(ctx) {
				return this.DB_Table1.updateMany(ctx, {
						surname: { $gt: ctx.params.minimum } // $gt -> all entities with 'third' field greater than
		...
		removeByThird: {
			...
			handler(ctx) {
				var surname = parseInt(ctx.params.surname);

				if (!isNaN(surname))
					return this.DB_Table1.removeMany(ctx, {
						surname: { $lt: ctx.params.surname } // $lt -> all entities with 'third' field less than
						})
						//.then( (res) => res )
						.catch( (err) => this.requestError(CodeTypes.UNKOWN_ERROR) );
				else
